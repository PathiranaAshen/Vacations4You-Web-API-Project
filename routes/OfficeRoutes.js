...
router.post("/add/cruise", upload.single("file"), async (req, res) => {
  // Check if a file is provided
  if (!req.file) {
    return res.status(400).send("No file uploaded.");
  }
  ...
  for (const row of rows) {
    if (row !== "") {
      ...
      try {
        const check = await Cruise.findOne({ name: name });
        if (check) {
          continue;
        }
...
router.get("/get/cruise/:id", getCruiseById);
router.get("/search/cruise/:key", getCruiseByKey);
router.get("/get/cruise", getCruise);
router.put("/update/cruise/:id", editCruise);
router.delete("/delete/cruise/:id", deleteCruise);
